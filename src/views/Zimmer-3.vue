  ...
  async checkAvailability(roomId) {
      if (this.fromDate && this.toDate) {
        try {
          ...
          console.log(response.data);
          this.availability[roomId] = response.data.available; // replace with the actual property name
        } catch (error) {
          console.error(error);
        }
      }
...
<template>
  <div class="container sm-8" align-self="stretch" name="intoduction">
    ...
    <form>
      ...
      <button @clicked="checkAvailability(room.id)">Verfügbarkeit prüfen</button>
    
      <div v-if="availability[room.id] === true">
      Das Zimmer ist verfügbar.
    </div>
    <div v-if="availability[room.id] === false">
