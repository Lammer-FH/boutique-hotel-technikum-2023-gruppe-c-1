...
export default {
  ...
  methods: {
    confirmBookingData() {
        ...
        selectRoomId: this.selectRoomId,
    ...
    async submitBooking(bookingData) {
	    if (this.email !== this.emailConfirm) {
	      ...
	      alert("Die E-Mail-Adressen stimmen nicht überein.")
      } if (this.dataPrivacyAggree == false) {
        alert("Die E-Mail-Adressen stimmen nicht überein.")
      } try {
	      this.bookingApi.postBooking(bookingData).then(() => {
	      this.validInput = true;
	      this.postBookingData(bookingData);
	    })
	    } catch (error) {
	      this.validInput = false;
	      error = "Es ist ein Fehler bei der Buchung aufgetreten.";
	      alert (error);
	      console.log(error);
	    }
    },
...
<template>
  <b-container fluid>
    <b-col><h1>Buchen Sie jetzt Ihr Zimmer</h1></b-col>
